...
class ControllerDashboard extends ControllerBase {
    ...
    public function dashboard() {
        ...
        $data['title'] = 'Rahail ';
		$data['price_info'] = $this->authentication->check_auth(array('ADMIN','RM'));;
	...
	public function getDashboardNotification(){
		...
		$rows =$this->modelOrders->processList("",['data_per_page'=>30],$filter);
		//printr($rows);
	...
	private function getFilter(){
		...
		$filter = '';
		if($isRM){
		 $filter = "(((place_orders.status=50) || (place_orders.status=150 || place_orders.status=220 || place_orders.status=320) ) && ((raiseby_id=$userid) || (rm_id=$userid)))";
		}
		if($isSV){
			$filter = "( (place_orders.status = 100) && (place_orders.supplier_id = $userid) )";
		}
		if($isDRV ){
			$filter = "(((place_orders.status=200) || (place_orders.status=250)) && ((driver_id=$userid) || (driver2_id=$userid)))";
		}
		if($isAdmin || $isMD ){
			$filter = "(place_orders.status between 50 and 320) and (place_orders.status != 300)";
		}
