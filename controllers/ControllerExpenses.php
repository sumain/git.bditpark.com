...
class ControllerExpenses extends ControllerBase {
    ...
    public function lists($pa = 1) {
        ...
        $data['scripts'][] = SCRIPT . base_url('assets/js/expenses/script.js') . END_SCRIPT;

        $result = $this->modelExpenses->expLists($filter, $page);
        $data['result'] = $result['rows'];
        $page['total_row'] = $result['ttl_rows'];
        $page['page_url'] = base_url() . "modelExpenses/lists/";
        ...
        $data['nav'] = array('1' => 'otherExp', '2' => 'expense', '3' => '');

        $data['main_content'] = 'not_authorized';
        $admin = $this->authentication->check_auth(array('ADMIN','OTREXP','MD'));
        if ( $admin) {
            $data['main_content'] = 'expenses/list';
        }
        $data['auth'] = $this->authentication->check_auth(['ADMIN','MD']);
    ...
    public function add() {
        ...
        $data['main_content'] = 'not_authorized';
        $admin = $this->authentication->check_auth(['ADMIN','MD','OTREXP']);
        if ($admin) {
            $data['main_content'] = 'expenses/buy_add';
        }
    ...
    public function create() {
        ...
        if ($action) {
            ...
            $data['userid']  = $userid;            
            $data['receipt'] = $this->modelExpenses->getOrderNo();
            ...
            $new_id = $this->modelExpenses->createNew($data);
			/*
            ...
            */
            ...
            foreach ($qty as $k => $val) {
                ...
                if ($val) {
                    ...
                    $did = $this->modelCommon->insertTableData($data, 'expenses_products');
                    if($val >=1){
						$invt['userid'] = $userid;
						$invt['productid'] = $pid;
						//$invt['locationid'] = 0;
			...
			$log['orderid'] = 0;
			$log['product'] = 0;
			$log['notes']   = 'Create Other Expense ';
    ...
    public function details($id) {
        ...
        $data['details'] = array();
        if ($id) {
            $data['details'] = $this->modelExpenses->getDetails($id);
       ...
       // $this->load->view('sales/detail_pr', $data);
		//printr($data);
		$data['path'] = $this->path;
		$data['main_content'] = 'expenses/detail_pr';
		$this->load->view(THEMES, $data);
    }
	...
	public function getProductBySku(){
		...
		if(!empty($result)){
			...
			if($sale==1){
				...
				$cont_row .='<td align="right"><input type="text" autocomplete="off" id="qty'.$counter.'" name="qty[]" value="1" class="form-control text-right qty "></td>'; 
