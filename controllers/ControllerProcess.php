...
class ControllerProcess extends ControllerBase {
	...
	protected $appstatus = [''=>'Status','100'=>'Packing','150'=>'Assaging for Pickup','250'=>'Deliver','300'=>'Delivered','320'=>'Cancel'];
    public function __construct() {
		...
		$this->approval= ['50'=>'RM','100'=>'SV','150'=>'RM','250'=>'DRV','300'=>'Delivered','320'=>'Cancel'];
		//$this->fields= ['50'=>'raiseby','100'=>'supplier','150'=>'rm','200'=>'driver','220'=>'rm2','250'=>'driver2','300'=>'Delivered','320'=>'Cancel'];
		$this->fields= ['50'=>'raiseby','100'=>'supplier','150'=>'rm','250'=>'driver2','300'=>'Delivered','320'=>'Cancel'];
    ...
    public function index() {
     ...
     public function lists($pa=1) {
		...
		$filter['products.product_id']= $product;
		$filter['fromdate']= $fromdate;
		$filter['todate']= $todate;
		if($isSV){
			$filter['place_orders.supplier_id']= $this->userid;
		}elseif($isDRV){
			$filter2= "((place_orders.driver_id = $this->userid) || (place_orders.driver2_id = $this->userid))";
		}elseif($isAdmin || $isMD){
			if($type ==2){
				$filter['place_orders.supplier_id']= $emp;
			}elseif($type ==3){
				$filter['place_orders.driver_id']= $emp;
			}
		}
		...
		$result            = $this->modelOrders->processList($filter,$page,$filter2);
		
		$data['result']    = $result['rows'];
		$page['total_row'] = $result['ttl_rows'];
		$page['page_url']  = base_url()."process/lists/";
		...
		if( $admin){
			$data['main_content'] = 'process/list';
		}
